  ...
  statusPollingInterval = setInterval(async () => {
    try {
      ...
      if (statusData.is_processing) {
        ...
        progressText.textContent = `Đang xử lý: ${statusData.completed}/${statusData.total} (${Math.round(percentage)}%)`;

        // Update current file and show individual progress
        updateIndividualFileProgress(statusData);

        if (statusData.current_file) {
          let currentFileText = `Đang xử lý: ${statusData.current_file}`;
                ...
                <div class="file-item">
                  ...
                  <div class="file-info" style="padding: 0.75rem; display: flex; flex-direction: column; gap: 0.5rem;">
                    <div style="display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                      <i class="fas fa-file-${result.file_type === 'image' ? 'image' : 'video'} file-icon"></i>
